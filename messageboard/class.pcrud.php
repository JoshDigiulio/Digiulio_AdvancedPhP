...
class pcrud
{
    ...
    //The function that is going to be used to create a new message in the message board by getting the message varaible from the add-message.php page:
    public function create($topic_id,$message)
    {   //Store the date that this message is created in the created varaible haveing the date be equal to the moment of creation.
        $created = date("Y-m-d H:i:s");
        //Try catch error checking time:
        try
        {
            //pPrepare this database to accept the sql stament below:
            $stmt = $this->db->prepare("INSERT INTO posts(topic_id,message,created) VALUES(:topic_id,:message,:created)");
            ...
            //Perform the requested stmt:
            $stmt->execute();
            //If all works return the value of true:
            return true;
        }
    ...
    //Functions used to get the post_ids:
    public function getID($id)
    {
        //Sql stament that prepare this database to gather all of the post ids:
        $stmt = $this->db->prepare("SELECT * FROM posts WHERE post_id=:id");
        //After the sql statment is execute place the post ids into a array and set the ids equal to one another:
    ...
    //Function used to display topics:
    public function displaytopics(){
    //Prepare this database to use this sql stmt:
    $stmt = $this->db->prepare("SELECT * FROM topics");
    //Execute the sql stmt:
    $stmt->execute();
    //Fetechs everything that is true:
    $rows=$stmt->fetchall();
    //Returns said rows that have been feteched:
    return $rows;
    }
    //Function used to update topic id's and the subject titles of those ids:
    public function update($post_id,$message)
    {   //Try and catch some problems time:
        try
        {
            //Preapre the database to take the sql stmt:
            $stmt=$this->db->prepare("UPDATE posts SET message=:message
    ...
    public function dataview($query)
    {
        ...
        if($stmt->rowCount()>0)
        {
            ...
            while($row=$stmt->fetch(PDO::FETCH_ASSOC))
            {
                ?>
                <tr>
                    <!-- Display the post id of each row: -->
                    <td><?php print($row['post_id']); ?></td>
                    <!-- Display the message of each row: -->
                    <td><?php print($row['message']); ?></td>
                    <!-- Display time created of each row: -->
                    <td><?php print($row['created']); ?></td>
                    <td align="center">
                        <!-- Pass the id of href into the href link: -->
                        <a href="edit-message.php?edit_id=<?php print($row['post_id']); ?>"><i class="glyphicon glyphicon-edit"></i></a>
                    </td>
                    <!-- Bootstrap Formatting -->
                    <td align="center">
                        <!-- Pass the id of href into the href link: -->
                        <a href="delete-message.php?delete_id=<?php print($row['post_id']); ?>"><i class="glyphicon glyphicon-remove-circle"></i></a>
                    </td>
                </tr>
                <?php
            }
